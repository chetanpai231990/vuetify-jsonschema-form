...
schemaUtils.getRules = (fullSchema, required, options) => {
  ...
  if (['number', 'integer'].includes(fullSchema.type) && fullSchema.maximum !== undefined) {
    rules.push((val) => (val === undefined || val === null || val <= fullSchema.maximum) || 'should be in range of min: '+fullSchema.minimum +' max:'+fullSchema.maximum)
  }
  if (['number', 'integer'].includes(fullSchema.type) && fullSchema.minimum !== undefined) {
    rules.push((val) => (val === undefined || val === null || val >= fullSchema.minimum) || 'should be in range of min: '+fullSchema.minimum +' max:'+fullSchema.maximum)
  }
  if (fullSchema.show_as === 'octet string' && fullSchema.minimum !== undefined) {
    rules.push((val) => (val === undefined || val === null || val.length >= fullSchema.minimum) || 'should be in range of min: '+fullSchema.minimum +' max:'+fullSchema.maximum)
  }
  if (fullSchema.show_as == 'octet string' && fullSchema.maximum !== undefined) {
    rules.push((val) => (val === undefined || val === null || val.length <= fullSchema.maximum) || 'should be in range of min: '+fullSchema.minimum +' max:'+fullSchema.maximum)
  }
  if (fullSchema.show_as == 'hex' && fullSchema.minimum !== undefined) {
    rules.push((val) => (val === undefined || val === null || (4) >= fullSchema.minimum || (val.length % 2) == 0) || 'should be in range of min: '+fullSchema.minimum +' max:'+fullSchema.maximum +'or  odd length ')
  }
  if (fullSchema.show_as == 'hex' && fullSchema.maximum !== undefined) {
    rules.push((val) => (val === undefined || val === null || (4) <= fullSchema.maximum || (val.length % 2) == 0) || 'should be in range of min: '+fullSchema.minimum +' max:'+fullSchema.maximum +'or  odd length ')
  }
  if (fullSchema.enum) {
...
schemaUtils.isOneOfSelect = (fullSchema) => {
  return (fullSchema.type === 'array' && ['string', 'integer', 'number'].includes(fullSchema.items.type) && fullSchema.items.choice) || (['string', 'integer', 'number'].includes(fullSchema.type) && fullSchema.choice)
}
